...
export class NekoClient extends BaseClient implements EventEmitter<any> {
  ...
  login(password: string, displayname: string) {
    const url =
      process.env.NODE_ENV === 'development'
        ...
        : `${/https/gi.test(location.protocol) ? 'wss' : 'ws'}://${
            location.host
          }/`;
  ...
  protected [EVENT.MEMBER.CONNECTED](member: MemberPayload) {
    ...
    if (member.id !== this.id) {
      this.$accessor.chat.newMessage({
        id: member.id,
        content: this.$vue.$t('notifications.connected', {
          name: '',
        }) as string,
        type: 'event',
        created: new Date(),
  ...
  protected [EVENT.MEMBER.DISCONNECTED]({ id }: MemberDisconnectPayload) {
    ...
    this.$accessor.chat.newMessage({
      id: member.id,
      content: this.$vue.$t('notifications.disconnected', {
        name: '',
      }) as string,
      type: 'event',
      created: new Date(),
  ...
  protected [EVENT.ADMIN.RELEASE]({ id, target }: AdminTargetPayload) {
    ...
    if (!target) {
      this.$accessor.chat.newMessage({
        id,
        content: this.$vue.$t(
          'notifications.controls_released_force'
        ) as string,
        type: 'event',
        created: new Date(),
      });
