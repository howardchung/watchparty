...
class VMManager {
        ...
        this.resetVM = async (vmid, roomId) => {
            ...
            if (this.reuseVMs) {
                ...
                if (this.imageId !== vmImageId) {
                    ...
                    (0, redis_1.redisCount)('vBrowserReimage');
    ...
    VALUES($1, $2, NOW(), 'staging', $3)`, [this.getPoolName(), id, this.imageId]);
        ...
        `, [this.getPoolName()]);
                ...
                for (let i = 0; i < allVMs.length; i++) {
                    ...
                    if (!inUse.has(server.id)) {
                        (0, redis_1.redisCount)('vBrowserCleanup');
        ...
        `, [this.getPoolName()]);
                const stagingPromises = rows.map(async (row) => {
                    ...
                    const shouldFetchVM = retryCount === this.minRetries + 1 || retryCount % 30 === 0;
                    ...
                    if (retryCount % 180 === 0) {
                        ...
                        await postgres.query(`UPDATE vbrowser SET image = NULL WHERE pool = $1 AND vmid = $2`, [this.getPoolName(), vmid]);
