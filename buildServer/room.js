    ...
    constructor(io, roomId, roomData) {
        ...
        this.saveRoom = async () => {
            ...
            if (postgres_1.postgres) {
                try {
                    ...
                    await postgres_1.postgres.query(`UPDATE room SET "lastUpdateTime" = $1, data = $2 WHERE "roomId" = $3`, [this.lastUpdateTime, roomString, this.roomId]);
                }
                catch (e) {
                    console.warn(e);
                }
        ...
        this.stopVBrowserInternal = async () => {
            ...
            if (redis_1.redis && assignTime) {
                await redis_1.redis.lpush('vBrowserSessionMS', Number(new Date()) - assignTime);
        ...
        this.joinScreenSharing = (socket, data) => {
            ...
            if (data?.mediasoup) {
                ...
                mediasoupSuffix = '@' + config_1.default.MEDIASOUP_SERVER + '/' + (0, uuid_1.v4)();
        ...
        io.of(roomId).use(async (socket, next) => {
            ...
            if (postgres_1.postgres) {
                const result = await postgres_1.postgres.query(`SELECT password, "isSubRoom" FROM room where "roomId" = $1`, [this.roomId]);
